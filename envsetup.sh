...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var XMOD_VERSION)
        ZIPFILE=xmod-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.xmod.device=$XMOD_BUILD");
    then
...
function installboot()
{
    ...
    then
        ...
        echo "The connected device does not appear to be $XMOD_BUILD, run away!"
    fi
}
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/xmod/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
